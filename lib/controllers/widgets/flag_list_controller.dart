...
class FlagListState {
  ...
  const FlagListState(this.collections,
      {this.fetchState = kdataFetchState.IS_LOADING, this.selectedFilter});
}

class FlagListController extends StateNotifier<FlagListState> {
  FlagListController(List<Collections> collectionsList)
      : super(FlagListState(collectionsList));
  ...
  Future<String> prepareFromInternet(Collections collections) async {
    ...
    var tokenURI;
    try {
      tokenURI = await erc.tokenURI(BigInt.parse(collections.tokenID));
    } catch (error) {
      print(error);
      collections.isNotSupported = true;
      collectionsDAO.update(collections);
      List<Collections>? collectionsArray = state.collections;
      collectionsArray[collectionsArray.indexWhere((element) =>
          element.contractAddress == collections.contractAddress &&
          element.tokenID == collections.tokenID)] = collections;

      state = FlagListState(collectionsArray,
          fetchState: state.fetchState, selectedFilter: state.selectedFilter);
      return "";
    }
    print(tokenURI);
    ...
    var collectionsItem = CollectionsItem(
        ...
        collections.hash,
        collections.tokenID,
        '${jsonData['name']} #${collections.tokenID}',
        image,
        attributes,
